"""Resize, compress, and store chat images as WebP."""
...
def _webp_quality() -> int:
    return int(getattr(settings, "BUBBLLE_IMAGE_WEBP_QUALITY", 80))


def chat_image_upload_path(bubble_id: uuid.UUID | str) -> str:
    return f"chat/{bubble_id}/{uuid.uuid4().hex}.webp"


def optimize_chat_image(uploaded_file) -> tuple[ContentFile, int, int]:
    """
    Strip EXIF, auto-orient, resize, encode WebP.
